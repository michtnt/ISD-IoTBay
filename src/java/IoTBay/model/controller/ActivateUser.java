...
public class ActivateUser extends HttpServlet 
{
    ...
    {
        String ID = request.getParameter("ID");//take ID from jsp
        response.setContentType("text/html;charset=UTF-8");
        try
        {
            Connector = new DBConnector();// set new connector
            Query = new DBManager(Connector.openConnection());//set DBManager and open connection
        ...
        try
        {
        Query.ActivateUser(ID);//Run Query from DBManager
